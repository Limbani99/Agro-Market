...
const ProductInfo = ({ product, reviewsCount = 0, averageRating = 4.8 }) => {
    const [quantity, setQuantity] = useState(1)
    const [added, setAdded] = useState(false)
    const { addToCart, toggleWishlist, isWishlisted } = useData()

    const sellerId = product.sellerId?._id || product.sellerId?.id || product.sellerId || "1";
    ...
    return (
        <div className="flex flex-col h-full">
            <div className="flex-1">
                <span className="text-primary font-bold text-sm uppercase tracking-wider mb-2 block">{product.category}</span>
                <h1 className="text-4xl lg:text-5xl font-black text-secondary mb-4">{product.name}</h1>

                <div className="flex flex-wrap items-center gap-4 mb-6">
                    <div className="flex items-center gap-1">
                        {[...Array(5)].map((_, i) => (
                            <Star key={i} className={`w-4 h-4 ${i < Math.floor(averageRating) ? 'fill-accent text-accent' : 'text-slate-200'}`} />
                        ))}
                    </div>
                    ...
                    <Link to={`/farmer/${sellerId}`} className="text-sm font-bold text-primary hover:underline flex items-center gap-1">
                ...
                <div className="flex flex-col sm:flex-row items-stretch gap-4 mb-8">
                    ...
                    <button
                        ...
                        className={`flex-1 px-8 py-4 rounded-2xl font-black flex items-center justify-center gap-3 shadow-xl transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed ${added
                    ...
                    <button
                        ...
                        className={`p-4 border-2 rounded-2xl transition-all cursor-pointer ${isWishlisted(product.id)
                                ? 'border-red-100 bg-red-50 text-red-500 hover:bg-red-100/70'
                                : 'border-slate-100 text-slate-400 hover:text-red-500 hover:border-red-100 hover:bg-red-50'
                            }`}
                    >
                        <Heart className={`w-6 h-6 ${isWishlisted(product.id) ? 'fill-red-500 text-red-500' : ''}`} />
                    </button>
                </div>
            </div>

            {/* Delivery Info */}
            <div className="bg-primary-light border border-primary/10 rounded-2xl p-5 flex items-start gap-4">
                ...
                <div>
                    ...
                    <p className="text-xs text-slate-500 font-medium leading-relaxed">
                        Order within <span className="text-primary font-bold">4 hours</span> to guarantee next-day arrival.
                    </p>
                </div>
            </div>
        </div>
